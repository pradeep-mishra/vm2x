...
return ((vm, host) => {
	...
	const _prepareRequire = (currentDirname, parentAllowsTransitive = false) => {
		const _require = moduleName => {
			if (vm.options.nesting && moduleName === 'vm2') return {
				VM: Contextify.readonly(host.VM),
				NodeVM: Contextify.readonly(host.NodeVM)
			};
			...
			if (/^(\.|\.\/|\.\.\/)/.exec(moduleName)) {
				...
				if (typeof vm.options.require.external === 'object') {
					const {
						external,
						transitive
					} = _parseExternalOptions(vm.options.require.external);
			...
			if ((vm.options.require.external.indexOf(moduleName) !== -1) &&
				(dirname.indexOf('/node_modules/') !== -1)) {
				let splits = dirname.substr(dirname.indexOf('node_modules/')).split(/[\/]+/);
				let mod = splits[1] || '';
				if ((moduleName === mod) && vm.options.require.external.indexOf(moduleName)) {
					module.exports = Contextify.readonly(host.require(filename));
					module.require = host.require;
					return module.exports;
				}
			}
